    ...
    internal class Game : GameWindow
    {
        ...
        protected override void OnLoad()
        {
            ...
            for (int i = 0; i < 4; i++)
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 4; k++)
                        Statics.Chunks[$"{i}:{j}:{k}"] = new Chunk(new Vector3(i, j, k));
            //for (int i = 0; i > -4; i--)
            //    for (int j = 0; j > -1; j--)
            //        for (int k = 0; k > 4; k--)
            //            Statics.Chunks[$"{i}:{j}:{k}"] = new Chunk(new Vector3(i, j, k));
        ...
        protected override void OnRenderFrame(FrameEventArgs args)
        {
            ...
            Console.Clear();
            if (1f / args.Time > max) max = 1d / args.Time;
            Title = $"(Vsync: {VSync}) FPS: {1f / args.Time:0} Max: {max:0} ";
            GL.ClearDepth(1);
            GL.ClearColor(0.3f, 0.3f, 1f, 1f);
            GL.Clear(ClearBufferMask.ColorBufferBit | ClearBufferMask.DepthBufferBit);
            var t = new Vector4(-1, 0, 0, 1);
            Matrix4 model = Matrix4.Identity;
            Matrix4 view = camera.GetViewMatrix();
            Matrix4 projection = camera.GetProjectionMatrix();
            int modelLocation = GL.GetUniformLocation(program3D.ID, "model");
            int viewLocation = GL.GetUniformLocation(program3D.ID, "view");
            int projectionLocation = GL.GetUniformLocation(program3D.ID, "projection");
            GL.UniformMatrix4(modelLocation, true, ref model);
            GL.UniformMatrix4(viewLocation, true, ref view);
            GL.UniformMatrix4(projectionLocation, true, ref projection);
            var x = (int)Statics.CameraLockTarget.X;
            var y = (int)Statics.CameraLockTarget.Y;
            var z = (int)Statics.CameraLockTarget.Z;
        ...
        public void GoBack(Matrix4 v, Matrix4 p, float y)
        {
            ...
            f1 = f1 - ((y * v.M21) + v.M41);
            f2 = f2 - ((y * v.M22) + v.M42);
            Matrix2 m1 = new Matrix2(v.M12, v.M32, v.M11, v.M31);
            Vector2 f = new Vector2(f2, f1);
        ...
        protected override void OnUpdateFrame(FrameEventArgs args)
        {
            ...
            if (input.IsKeyDown(Keys.Z))
                foreach (var chunk in Statics.Chunks.Values)
                {
                    chunk.GenBlocks();
                    chunk.BuildChunk();
                }
